...
|---|---|---|
| 弹幕 | `DANMU_MSG` | `[直播间] 弹幕：昵称说「内容」`（多条合并时编号列表） |
| 礼物 | `SEND_GIFT` | `[直播间] 礼物：昵称送出 礼物名 xN（价值¥N）` |
| 醒目留言 | `SUPER_CHAT_MESSAGE` | `[直播间] 醒目留言：昵称留言「内容」（¥N）` |
| 互动 | `INTERACT_WORD_V2` | `[直播间] 互动：昵称关注了主播 / 特别关注了主播 / 和主播互粉了 / 分享了直播间` |

说明：

- **进场事件**（`msg_type=1`）不响应——热门房间进场太频繁，会刷屏。
- `INTERACT_WORD_V2` 是互动事件（关注/分享等），不是弹幕；弹幕只走 `DANMU_MSG`。
- 所有消息都带 `[直播间]` 前缀，提示模型这不是用户本人输入。
...
2. **最小间隔**：两次 `SendChatAsync` 之间至少间隔 `minIntervalMs`，期间新事件继续排队。
